...
<body>
        ...
        <div class="nav-container">
            <nav class="nav-fixed">
              <ul>
                ...
                <li><a href="profile.php?logout=1"><i class="fa fa-power-off" aria-hidden="true"></i>Logout</a></li>
              </ul>
            </nav>
        </div>
    </aside>
  ...
  <article id="profile-article">
    ...
    <?php        
                    $db = mysqli_connect("localhost","root","","sde");
                    $userOper = $_SESSION['userOper'];
                    $viewUser = "SELECT fullname, brgy, city, cellp, emailOper FROM registeroperator WHERE userOper = '$userOper'";
                    $result = mysqli_query($db, $viewUser);
                    if (mysqli_num_rows($result)> 0) {
          ...
          <div class="user-bg">
            ...
            <div class="overlay-box">
              ...
              <?php 
                ...
                if (mysqli_num_rows($resultimg)>0) {
                  while($row3 = mysqli_fetch_assoc($resultimg)){
                    echo "<img class='thumb-lg  img-circle' src='operator-image/". $_SESSION['userOper'] . "/".$row3['oper_image']."'>";  
                  }
                }
              ?>

                <h4 class="text-white"><?php echo $userOper; ?></h4>
                <h5 class="text-white"><?php echo $row2['emailOper']; ?></h5>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="white-box2">
          <form id="update" method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>" class="form-horizontal  form-material">
            <div class="form-group">
              <label>Full Name</label><br>
              <input id="fullname" type="text" name="fullname" placeholder="<?php echo $row2['fullname'];?>" class="form-control  form-control-line" value="<?php echo $row2['fullname'];?>">
            </div>

             <div class="form-group">
              <label>Email</label><br>
              <input id="emailOper" type="text" name="emailOper" placeholder="<?php echo $row2['emailOper'];?>" class="form-control  form-control-line" value="<?php echo $row2['emailOper'];?>">
            </div>

             <div class="form-group">
              <label>Phone Number</label><br>
              <input id="cellp" type="text" name="cellp" placeholder="<?php echo $row2['cellp'];?>" class="form-control  form-control-line" value="<?php echo $row2['cellp'];?>">
            </div>

             <div class="form-group">
              <label>Address</label><br>
              <input id="brgy" type="text" name="brgy" placeholder="<?php echo $row2['brgy']; ?>" class="form-control  form-control-line" value="<?php echo $row2['brgy'];?>">
              <input id="city" type="text" name="city" placeholder="<?php echo $row2['city']; ?>" class="form-control  form-control-line" value="<?php echo $row2['city'];?>">
            </div>

             <div class="form-group">
              <input type="submit" name="updateOper" value="Update Profile" class="btn  btn-success">
            </div>
          </form>
        </div>
        ...
        <div class="white-box3">
          <div class="user-info">
            <h4><i class="fa fa-info" aria-hidden="true"></i><?php echo $row2['fullname'];  ?></h4>
            <h4><i class="fa fa-map-marker" aria-hidden="true"></i><?php echo $row2['brgy']; ?> , <?php echo
             $row2['city'];?></h4>
            <h4><i class="fa fa-phone" aria-hidden="true"></i><?php echo $row2['cellp']; ?></h4>
      ...
      ?>
        ...
        if (isset($_POST['upload'])) {
            $db = mysqli_connect("localhost","root","","sde");
            $target = "operator-image/". $_SESSION['userOper'] . "/". basename($_FILES['image']['name']);

            $image = $_FILES['image']['name'];

            $sql = "UPDATE registeroperator SET oper_image ='$image' WHERE userOper = '$userOper'";
            mysqli_query($db, $sql);

            if(!is_dir("operator-image/". $_SESSION['userOper']."/")){
                mkdir("operator-image/". $_SESSION['userOper']."/");
            }
  ...
  <script> 
    $(document).ready(function(){
          $("#update").submit(function(e) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "../php/registerup.php",
                        data: "fullname=" + fullname + "&emailOper=" + emailOper + "&cellp=" + cellp + "&brgy=" + brgy + "&city=" + city,
